    ...
    while True:
        try:
            ...
            subprocess.check_call(
                'split -l 25000000 {0} {0}-chunk_'.format(path), shell=True)
        except Queue.Empty:
            return
    ...
    while True:
        try:
            ...
            subprocess.check_call("sort -T /home/cidr_analysis/tmp/ " +
                "-t ',' -s -k 1,1 -o {1} {0}".format(path, out_path),
                shell=True)
    ...
    while True:
        try:
            ...
            subprocess.check_call("sort -m -T /home/cidr_analysis/tmp/ " +
                "-t ',' -s -k 1,1 -o {2} {0} {1}".format(
                path_1, path_2, out_path), shell=True)
            subprocess.check_call('rm {0} {1}'.format(path_1, path_2),
                shell=True)
            out_queue.put_nowait(out_path)
        except Queue.Empty:
            if path_1:
